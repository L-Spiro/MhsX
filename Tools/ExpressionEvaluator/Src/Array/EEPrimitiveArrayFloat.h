...
namespace ee {
	...
	public :
		...
		virtual bool								Initialize( const CExpEvalContainer::EE_RESULT &_rStart, const CExpEvalContainer::EE_RESULT &_rEnd ) {
			...
			if ( m_vArray.size() == 1 ) {
				CExpEvalContainer::EE_RESULT rTemp = CExpEvalContainer::ConvertResult( _rStart, EE_NC_FLOATING );
		...
		virtual bool								WriteValue( size_t _sIdx, CExpEvalContainer::EE_RESULT &_rRet ) {
			...
			CExpEvalContainer::EE_RESULT rTemp = CExpEvalContainer::ConvertResult( _rRet, EE_NC_FLOATING );
