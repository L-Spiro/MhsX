...
namespace lson {
	...
	public :
		...
		// == Types.
		/** The node type. */
		typedef CTree<_tType>							LSON_NODE_TYPE;

		/** The pointer to each node. */
		typedef std::unique_ptr<LSON_NODE_TYPE>			LSON_PTR_TYPE;
		...
		std::vector<LSON_PTR_TYPE>::iterator			InsertChild_It( const _tType &_tChild, std::vector<LSON_PTR_TYPE>::iterator _iIt ) {
	...
	protected :
		...
		/** Children. */
		std::vector<LSON_PTR_TYPE>						m_vChildren;
