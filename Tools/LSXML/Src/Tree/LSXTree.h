...
namespace lsx {
	...
	public :
		...
		// == Types.
		/** The node type. */
		typedef CTree<_tType>							LSX_NODE_TYPE;

		/** The pointer to each node. */
		typedef std::unique_ptr<LSX_NODE_TYPE>			LSX_PTR_TYPE;
		...
		std::vector<LSX_PTR_TYPE>::iterator				InsertChild_It( const _tType &_tChild, std::vector<LSX_PTR_TYPE>::iterator _iIt ) {
	...
	protected :
		...
		/** Children. */
		std::vector<LSX_PTR_TYPE>						m_vChildren;
