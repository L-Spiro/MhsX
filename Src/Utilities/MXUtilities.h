...
namespace mx {
	...
	public :
		...
		struct MX_TIMER {
			MX_TIMER( HWND _hWnd, UINT_PTR _nIDEvent, UINT _uElapse, TIMERPROC _lpTimerFunc ) :
				hWnd( _hWnd ),
				uiptrEvent( _nIDEvent ),
				uiTime( _uElapse ),
				lpTimerFunc( _lpTimerFunc ) {
				uiptrActiveId = CSystem::SetTimer( _hWnd, _nIDEvent, _uElapse, _lpTimerFunc );
			}
			MX_TIMER() {}
			~MX_TIMER() {
				Stop();
			...
			UINT_PTR							Start( HWND _hWnd, UINT_PTR _nIDEvent, UINT _uElapse, TIMERPROC _lpTimerFunc ) {
				...
				uiTime = _uElapse;
				lpTimerFunc = _lpTimerFunc;
				uiptrActiveId = CSystem::SetTimer( hWnd, uiptrEvent, uiTime, lpTimerFunc );
			...
			BOOL								Stop() {
				if ( uiptrActiveId ) {
					if ( CSystem::KillTimer( hWnd, uiptrActiveId ) ) {
						uiptrActiveId = 0;
						return TRUE;
					}
					return FALSE;
