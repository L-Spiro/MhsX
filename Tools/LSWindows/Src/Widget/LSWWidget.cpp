...
#undef LSW_ERROR_PRINT
	 ...
	 */
	CWidget::LSW_HANDLED CWidget::Char( UINT _uiKeyCode, UINT _uiFlags ) {
		const uint32_t uiRepeat = static_cast<uint32_t>(_uiFlags & 0xFFFFU);
		const wchar_t wcUnit = static_cast<wchar_t>(_uiKeyCode);

		// Common controls in WM_CHAR.
		if ( wcUnit == 0x08 ) { return LSW_H_CONTINUE; }    // Backspace: handle in KeyDown, usually.
		if ( wcUnit == 0x1B ) { return LSW_H_CONTINUE; }    // Escape.
		// Allow tab/enter if you want them as input.
		// if ( wcUnit == L'\t' || wcUnit == L'\r' ) ...

		// Surrogate handling.
		if ( ee::CExpEval::IsHighSurrogate( wcUnit ) ) {
			m_wcPendingHighSurrogate = wcUnit;
			return LSW_H_HANDLED;
		}
		if ( ee::CExpEval::IsLowSurrogate( wcUnit ) ) {
			if ( m_wcPendingHighSurrogate ) {
				const char32_t c32Cp = ee::CExpEval::CombineSurrogates( m_wcPendingHighSurrogate, wcUnit );
				m_wcPendingHighSurrogate = 0;
				return OnTextInputCodePoint( c32Cp, uiRepeat ? uiRepeat : 1 );
			}
			// Unpaired low surrogate: treat as replacement char or ignore.
			m_wcPendingHighSurrogate = 0;
			return OnTextInputCodePoint( 0xFFFD, uiRepeat ? uiRepeat : 1 );
		}

		// If a high surrogate was pending but we got a non-low, flush it as replacement.
		if ( m_wcPendingHighSurrogate ) {
			m_wcPendingHighSurrogate = 0;
			OnTextInputCodePoint( 0xFFFD, 1 );
		}

		const char32_t c32Cp = static_cast<char32_t>(wcUnit);
		if ( m_bDiscardControlChars ) {
			if ( ExcludeCHaracter( c32Cp ) ) { return LSW_H_CONTINUE; }
		}
	 ...
	 */
	CWidget::LSW_HANDLED CWidget::UniChar( UINT _uiKeyCode, UINT _uiFlags ) {
		static_cast<void>(_uiFlags);

		if ( _uiKeyCode == UNICODE_NOCHAR ) { return LSW_H_HANDLED; }

		const char32_t c32Cp = static_cast<char32_t>(_uiKeyCode);

		if ( m_bDiscardControlChars ) {
			if ( ExcludeCHaracter( c32Cp ) ) { return LSW_H_CONTINUE; }
		}

		return OnTextInputCodePoint( c32Cp, 1 );
	}
	 ...
	 */
	CWidget::LSW_HANDLED CWidget::ImeChar( UINT _uiKeyCode, UINT _uiFlags ) {
		static_cast<void>(_uiKeyCode);
		static_cast<void>(_uiFlags);

		// Prefer WM_IME_COMPOSITION + GCS_RESULTSTR to avoid double-inserts.
		return LSW_H_HANDLED;
	}
	 ...
	 */
	CWidget::LSW_HANDLED CWidget::ImeComposition( UINT _uiKeyCode, UINT _uiFlags ) {
		static_cast<void>(_uiKeyCode);
		try {
			// Only handle committed text here.
			if ( (_uiFlags & GCS_RESULTSTR) == 0 ) { return LSW_H_CONTINUE; }

			HIMC hImc = ::ImmGetContext( Wnd() );
			if ( !hImc ) { return LSW_H_CONTINUE; }

			const LONG lBytes = ::ImmGetCompositionStringW( hImc, GCS_RESULTSTR, nullptr, 0 );
			if ( lBytes <= 0 ) {
			...
			wchar_t wcPending = 0;
			for ( size_t I = 0; vW[I]; ++I ) {
				const wchar_t wcUnit = vW[I];
				...
				if ( ee::CExpEval::IsLowSurrogate( wcUnit ) ) {
					if ( wcPending ) {
						vCp.push_back( ee::CExpEval::CombineSurrogates( wcPending, wcUnit ) );
						wcPending = 0;
					}
					else {
						vCp.push_back( 0xFFFD );
					}
					continue;
				}
			...
			if ( wcPending ) { vCp.push_back( 0xFFFD ); }

			return OnTextInputCodePoints( vCp.data(), vCp.size() );
		}
		catch ( ... ) { return LSW_H_CONTINUE; }
	}
	...
	//	if ( HasGroupBoxChild( Wnd() ) ) { return LSW_H_CONTINUE; }
	//	/*HWND hChild = ::GetWindow( Wnd(), GW_CHILD );
	//	while ( hChild ) {
	//		if ( !IsGroupBox( hChild ) ) { return LSW_H_CONTINUE; }
	...
	//	// Clip out child windows so we donft paint under them.
	//	HWND hChild = ::GetWindow( Wnd(), GW_CHILD );
	//	while ( hChild ) {
...
//					POINT pConvClient = PixelsToDialogUnits( _hWnd, 290, 47 );
			...
			case WM_NOTIFY : {
				...
				switch ( lpHdr->code ) {
					...
					case LVN_GETDISPINFOW : {
						...
						HWND hFrom = plvdiInfo->hdr.hwndFrom;
						CWidget * pmwTemp = LSW_WIN2CLASS( hFrom );
						if ( plvdiInfo->item.pszText && plvdiInfo->item.cchTextMax > 0 ) {
			...
			case WM_CHAR : {
				LSW_HANDLED hHandled = pmwThis->Char( static_cast<UINT>(_wParam), static_cast<UINT>(_lParam) );
			...
			case WM_UNICHAR : {
				LSW_HANDLED hHandled = pmwThis->UniChar( static_cast<UINT>(_wParam), static_cast<UINT>(_lParam) );

				// Return value
				//	If wParam == UNICODE_NOCHAR, return nonzero if the application processes WM_UNICHAR characters.
				//	Otherwise, an application should return zero if it processes this message.
				if ( static_cast<UINT>(_wParam) == UNICODE_NOCHAR ) {
					if ( hHandled == LSW_H_HANDLED ) { LSW_RET( 1, 1 ); }
				}
				else {
					if ( hHandled == LSW_H_HANDLED ) { LSW_RET( 0, 0 ); }
			...
			case WM_IME_CHAR : {
				LSW_HANDLED hHandled = pmwThis->ImeChar( static_cast<UINT>(_wParam), static_cast<UINT>(_lParam) );
			...
			case WM_IME_COMPOSITION : {
				LSW_HANDLED hHandled = pmwThis->ImeComposition( static_cast<UINT>(_wParam), static_cast<UINT>(_lParam) );
			...
			case WM_IME_COMPOSITIONFULL : {
				LSW_HANDLED hHandled = pmwThis->ImeCompositionFull();
			...
			case WM_IME_CONTROL : {
				LSW_HANDLED hHandled = pmwThis->ImeControl( static_cast<UINT>(_wParam), static_cast<LPARAM>(_lParam) );
			...
			case WM_IME_STARTCOMPOSITION : {
				LSW_HANDLED hHandled = pmwThis->ImeStartComposition();
			...
			case WM_IME_ENDCOMPOSITION : {
				LSW_HANDLED hHandled = pmwThis->ImeEndComposition();
			...
			case WM_IME_KEYDOWN : {
				LSW_HANDLED hHandled = pmwThis->ImeKeyDown( static_cast<UINT>(_wParam), static_cast<UINT>(_lParam) );
			...
			case WM_IME_KEYUP : {
				LSW_HANDLED hHandled = pmwThis->ImeKeyUp( static_cast<UINT>(_wParam), static_cast<UINT>(_lParam) );
			...
			case WM_IME_NOTIFY : {
				LSW_HANDLED hHandled = pmwThis->ImeNotify( static_cast<UINT>(_wParam), static_cast<LPARAM>(_lParam) );
			...
			case WM_IME_REQUEST : {
				LSW_HANDLED hHandled = pmwThis->ImeRequest( static_cast<UINT>(_wParam), static_cast<LPARAM>(_lParam) );
			...
			case WM_IME_SELECT : {
				LSW_HANDLED hHandled = pmwThis->ImeSelect( static_cast<BOOL>(_wParam), static_cast<LPARAM>(_lParam) );
			...
			case WM_IME_SETCONTEXT : {
				LSW_HANDLED hHandled = pmwThis->ImeSetContext( static_cast<BOOL>(_wParam), static_cast<UINT>(_lParam) );
