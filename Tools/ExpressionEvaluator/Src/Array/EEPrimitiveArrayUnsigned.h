...
namespace ee {
	...
	class CPrimitiveArrayUnsigned : public CArrayBase {
	public :
		CPrimitiveArrayUnsigned( CExpEvalContainer * _peecContainer ) :
		...
		virtual bool								Initialize( const CExpEvalContainer::EE_RESULT &_rStart, const CExpEvalContainer::EE_RESULT &_rEnd ) {
			...
			if ( m_vArray.size() == 1 ) {
				CExpEvalContainer::EE_RESULT rTemp = m_peecContainer->ConvertResultOrObject( _rStart, EE_NC_UNSIGNED );
				if ( rTemp.ncType == EE_NC_INVALID ) { return false; }
				m_vArray[0] = static_cast<_tnBaseType>(rTemp.u.ui64Val);
			}
			else {
		...
		virtual bool								ReadValue( size_t _sIdx, CExpEvalContainer::EE_RESULT &_rRet ) {
			...
			_rRet.u.ui64Val = m_vArray[_sIdx];
			return true;
		}
		...
		virtual bool								WriteValue( size_t _sIdx, CExpEvalContainer::EE_RESULT &_rRet ) {
			...
			CExpEvalContainer::EE_RESULT rTemp = m_peecContainer->ConvertResultOrObject( _rRet, EE_NC_UNSIGNED );
			if ( rTemp.ncType == EE_NC_INVALID ) { return false; }
			m_vArray[_sIdx] = static_cast<_tnBaseType>(rTemp.u.ui64Val);
			_rRet.ncType = EE_NC_UNSIGNED;
			_rRet.u.ui64Val = m_vArray[_sIdx];
			return true;
		}
