...
namespace mx {
	...
	bool CMemHack::SaveSettings( lson::CJson::LSON_ELEMENT * _peJson, CStream * _psBinary, const MX_OPTIONS &_oOptions ) const {
		...
		try {
			...
			if ( _peJson ) {
				...
				_peJson->vObjectMembers.push_back( std::make_unique<lson::CJson::LSON_ELEMENT>() );
				lson::CJson::CreateObjectElement( _DEC_S_CAB443AC_EditSettings, (*_peJson->vObjectMembers[_peJson->vObjectMembers.size()-1]) );
				if ( !SaveEditSettings( _peJson->vObjectMembers[_peJson->vObjectMembers.size()-1].get(), _psBinary, _oOptions ) ) { return false; }
			}
			else {
				...
				if ( !SaveEditSettings( nullptr, _psBinary, _oOptions ) ) { return false; }
			}
		}
		catch ( ... ) {
			return false;
	...
	bool CMemHack::LoadSettings( lson::CJson * _pjJson, CStream * _psBinary, MX_OPTIONS &_oOptions ) {
		...
		if ( _pjJson ) {
			...
			pjvVal = _pjJson->GetContainer()->GetMemberByName( jvRoot, _DEC_S_CAB443AC_EditSettings );
			if ( !pjvVal || !LoadEditSettings( pjvVal, _pjJson->GetContainer(), nullptr, _oOptions, ui32Version ) ) { return false; }
		}
		else {
			...
			if ( !LoadEditSettings( nullptr, nullptr, _psBinary, _oOptions, ui32Version ) ) { return false; }
		}
		return true;
	}
	...
	bool __stdcall CMemHack::ExpAddressHandler( uint64_t _ui64Address, ee::EE_CAST_TYPES _ctType, uintptr_t _uiptrData, ee::CExpEvalContainer * _peecContainer, ee::CExpEvalContainer::EE_RESULT &_rResult ) {
		...
		size_t sOffset;
		CUtilities::MX_BYTESWAP bsByteSwap = static_cast<CUtilities::MX_BYTESWAP>(_mhMemHack->Options().bsByteswap);
		switch ( _ctType ) {
			case ee::EE_CT_INT8 : {
				_rResult.ncType = ee::EE_NC_SIGNED;
				int8_t i8Val;
				if ( _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &i8Val, sizeof( i8Val ), sOffset, bsByteSwap, NULL ) ) {
			...
			case ee::EE_CT_INT16 : {
				_rResult.ncType = ee::EE_NC_SIGNED;
				int16_t i16Val;
				if ( _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &i16Val, sizeof( i16Val ), sOffset, bsByteSwap, NULL ) ) {
			...
			case ee::EE_CT_INT32 : {
				_rResult.ncType = ee::EE_NC_SIGNED;
				int32_t i32Val;
				if ( _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &i32Val, sizeof( i32Val ), sOffset, bsByteSwap, NULL ) ) {
			...
			case ee::EE_CT_INT64 : {
				_rResult.ncType = ee::EE_NC_SIGNED;
				if ( _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &_rResult.u.i64Val, sizeof( _rResult.u.i64Val ), sOffset, bsByteSwap, NULL ) ) {
					return true;
				}
				break;
			}
			...
			case ee::EE_CT_UINT8 : {
				...
				return _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &_rResult.u.ui64Val, sizeof( uint8_t ), sOffset, bsByteSwap, NULL );
			}
			case ee::EE_CT_UINT16 : {
				_rResult.ncType = ee::EE_NC_UNSIGNED;
				_rResult.u.ui64Val = 0;
				return _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &_rResult.u.ui64Val, sizeof( uint16_t ), sOffset, bsByteSwap, NULL );
			}
			case ee::EE_CT_UINT32 : {
				_rResult.ncType = ee::EE_NC_UNSIGNED;
				_rResult.u.ui64Val = 0;
				return _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &_rResult.u.ui64Val, sizeof( uint32_t ), sOffset, bsByteSwap, NULL );
			}
			case ee::EE_CT_UINT64 : {
				_rResult.ncType = ee::EE_NC_UNSIGNED;
				_rResult.u.ui64Val = 0;
				return _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &_rResult.u.ui64Val, sizeof( uint64_t ), sOffset, bsByteSwap, NULL );
			}
			...
			case ee::EE_CT_FLOAT16 : {
				...
				uint16_t ui16Val = 0;
				if ( _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &ui16Val, sizeof( ui16Val ), sOffset, bsByteSwap, NULL ) ) {
			...
			case ee::EE_CT_FLOAT : {
				_rResult.ncType = ee::EE_NC_FLOATING;
				float fVal = 0.0f;
				if ( _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &fVal, sizeof( fVal ), sOffset, bsByteSwap, NULL ) ) {
			...
			case ee::EE_CT_DOUBLE : {
				_rResult.ncType = ee::EE_NC_FLOATING;
				_rResult.u.dVal = 0;
				return _mhMemHack->Process().ReadProcessMemory_PreProcessed( _ui64Address, &_rResult.u.dVal, sizeof( _rResult.u.dVal ), sOffset, bsByteSwap, NULL );
			}
		}
...
#undef MX_UNSIGNED
	...
	bool CMemHack::SaveSearchSettings( lson::CJson::LSON_ELEMENT * _peJson, CStream * _psBinary, const MX_OPTIONS &_oOptions ) const {
		if ( _peJson ) {
			...
			{
				_peJson->vObjectMembers.push_back( std::make_unique<lson::CJson::LSON_ELEMENT>() );
				lson::CJson::CreateStringElement( _DEC_S_19280E4E_From, ee::CExpEval::ToJsonString( ee::CExpEval::ToUtf8( _oOptions.wsFromText ) ), (*_peJson->vObjectMembers[_peJson->vObjectMembers.size()-1]) );
			}
			{
				CSecureWString swsTmp = CUtilities::EscapeNonJson( _oOptions.wsToText, false );
				_peJson->vObjectMembers.push_back( std::make_unique<lson::CJson::LSON_ELEMENT>() );
				lson::CJson::CreateStringElement( _DEC_S_4203F666_To, ee::CExpEval::ToJsonString( ee::CExpEval::ToUtf8( _oOptions.wsToText ) ), (*_peJson->vObjectMembers[_peJson->vObjectMembers.size()-1]) );
			}
				...
				for ( size_t I = 0; I < _oOptions.vFromHistory.size(); ++I ) {
					{
						...
						lson::CJson::CreateStringElement( std::to_string( I ), ee::CExpEval::ToJsonString( ee::CExpEval::ToUtf8( _oOptions.vFromHistory[I] ) ), (*peArray->vObjectMembers[peArray->vObjectMembers.size()-1]) );
					}
				}
			}
			...
			lson::CJson::CreateArrayElement( _DEC_S_B517206A_ToHistory, (*_peJson->vObjectMembers[_peJson->vObjectMembers.size()-1]) );
			{
				...
				for ( size_t I = 0; I < _oOptions.vToHistory.size(); ++I ) {
					{
						peArray->vObjectMembers.push_back( std::make_unique<lson::CJson::LSON_ELEMENT>() );
						lson::CJson::CreateStringElement( std::to_string( I ), ee::CExpEval::ToJsonString( ee::CExpEval::ToUtf8( _oOptions.vToHistory[I] ) ), (*peArray->vObjectMembers[peArray->vObjectMembers.size()-1]) );
					}
				}
			}
	...
	// Saves to JSON format if _peJson is not nullptr, otherwise it saves to binary stored in _psBinary.
	bool CMemHack::SaveEditSettings( lson::CJson::LSON_ELEMENT * _peJson, CStream * _psBinary, const MX_OPTIONS &_oOptions ) const {
		if ( _peJson ) {
			_peJson->vObjectMembers.push_back( std::make_unique<lson::CJson::LSON_ELEMENT>() );
			lson::CJson::CreateArrayElement( _DEC_S_D9701309_EditDescrHistory, (*_peJson->vObjectMembers[_peJson->vObjectMembers.size()-1]) );
			{
				lson::CJson::LSON_ELEMENT * peArray = _peJson->vObjectMembers[_peJson->vObjectMembers.size()-1].get();
				for ( size_t I = 0; I < _oOptions.vEditDescriptionHistory.size(); ++I ) {
					{
						peArray->vObjectMembers.push_back( std::make_unique<lson::CJson::LSON_ELEMENT>() );
						lson::CJson::CreateStringElement( std::to_string( I ), ee::CExpEval::ToJsonString( ee::CExpEval::ToUtf8( _oOptions.vEditDescriptionHistory[I] ) ), (*peArray->vObjectMembers[peArray->vObjectMembers.size()-1]) );
					}
				}
			}
