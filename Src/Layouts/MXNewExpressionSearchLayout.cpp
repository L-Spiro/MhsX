...
#undef MX_SEARCH_W
	...
	DWORD CNewExpressionSearchLayout::CreateNewExpressionSearchDialog( CWidget * _pwParent, CMemHack * _pmhMemHack ) {
		...
		CLayoutManager::UnencryptLayouts( m_wlNewExpressionSearchDialog, MX_ELEMENTS( m_wlNewExpressionSearchDialog ),
			vLayouts,
			sStringsW,
			sStrings );
		
		mx::CLayoutManager * plmLayout = static_cast<mx::CLayoutManager *>(lsw::CBase::LayoutManager());
		INT_PTR ipProc = plmLayout->DialogBoxX( &vLayouts[0], MX_ELEMENTS( m_wlNewExpressionSearchDialog ), _pwParent, reinterpret_cast<uint64_t>(_pmhMemHack) );
